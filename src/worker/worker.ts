...
ctx.onmessage = (event) => {
  ...
  switch (action.type) {
    case 'SEARCH':
      ...
      if (mode === 'js') { searchTinymtSeedJS(natures, hasShinyCharm) } else if (mode === 'wasm') { searchTinymtSeedWASM(natures, hasShinyCharm) } else { throw new Error(`Invalid mode: ${mode}`) }
      break
